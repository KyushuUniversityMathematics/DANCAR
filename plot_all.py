...
def plot_all(disks,min_radius,args):
    ...
    for i,v in enumerate(disks):
        ...
        c = patches.Circle(xy=xy, radius=v[0]-min_radius+1, fc=cmap(int(i%10)),alpha=0.4)
        ax.add_patch(c)
        ax.text(xy[0], xy[1], i, size = 20, color = cmap(int(i%10)))
        c = patches.Circle(xy=xy, radius=v[0]-min_radius+1, ec='black', fill=False)
        ax.add_patch(c)
    ...
    if args.save == True:
        plt.savefig(f"{args.folder_path}/plot.png")
    else:
        plt.show()
    plt.close()
...
def plot_edge(edges,disks,min_radius,args,folder_name):
    ...
    for edge in edges:
        ...
        for node in edge:
            ...
            c = patches.Circle(xy=(node_x,node_y), radius=radius-min_radius+1, fc=cmap(int(node%10)),alpha=0.4)
            ax.add_patch(c)
...
def main():
    ...
    disks = load_csv(f"{args.folder_path}/out.csv",float)
    min_radius = min(disk[0] for disk in disks)
