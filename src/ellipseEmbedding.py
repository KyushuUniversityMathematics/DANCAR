...
class Updater(chainer.training.StandardUpdater):
    ...
    def update_core(self):
        epsilon = 1e-10
        dim = round((len(self.coords.W[:,0]) - 1) / 2)
        opt = self.get_optimizer('main')
        r = F.relu(self.coords.W[:,0])+0.1 #FIXME

        # positive sample: a contains b
        batch = self.get_iterator('main').next()
        a,b = self.converter(batch)
        # d = F.sqrt(F.sum((self.coords.W[a,1:]-self.coords.W[b,1:])**2,axis=1)+epsilon)#FIXME
        # loss_pos = F.average(F.relu(d + self.args.margin + self.args.dag * r[b] - r[a]))#FIXME
        W_b = (self.coords.W[b,1:dim] + self.coords.W[b,dim+1:]) / 2
        W_a1 = self.coords.W[a,1:dim]
        W_a2 = self.coords.W[a,dim+1:]
        d = F.sqrt(F.sum((W_a1-W_b)**2,axis=1)+epsilon) + F.sqrt(F.sum((W_a2-W_b)**2,axis=1)+epsilon)
        loss_pos = F.average(F.relu(d + self.args.margin - r[a]))
        ...
        if self.args.lambda_coulomb>0:#FIXME
...
#    print(edge)
    ...
    pred = {v:{u for u,v_ in edge if v_ == v} for v in vert}

    return len(vert),list(edge),pred
...
#    vnum,pos_edge,neg_edge,super_neg_edge = read_graph(args.input)
    vnum,pos_edge,pred=read_graph(args.input)
