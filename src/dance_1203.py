...
class Updater(chainer.training.StandardUpdater):
    ...
    def update_core(self):
        ...
        loss_vert = self.args.lambda_vert * F.average(distance)
        ...
        if self.args.lambda_coulomb>0:
            ...
            # 円盤と円盤を離すようにする、総当たりでやる
            p = np.concatenate([np.random.permutation(len(r)) for _ in range(self.args.repeat_coulomb)])
            a,b = p[1:], p[:-1]
            ...
            # d = F.sqrt(F.sum((c[p[1:]]-c[p[:-1]])**2,axis=1)+epsilon)
            # loss_coulomb = -F.average((self.coords.W[p[1:],1:]-self.coords.W[p[:-1],1:])**2)
            # loss_coulomb = F.average(F.relu(r[p[1:]] + r[p[:-1]] - d + margin) )
...
def main():
    ...
    parser.add_argument('--lambda_vert','-lv',help="",type=float, default=0.5)
    ...
    parser.add_argument('--repeat_coulomb', '-rc', help='repeat coulomb', type=int, default=1)
    ...
    neg = (args.lambda_neg > 0)
    vnum, vert, pos_edge, neg_edge = read_graph(args.input, neg)
    # print(pos_edge)
